...
real dx, c, fmax, tmax, dt, df, pi, ndt, alpha
integer nx, nt, nf, ix, it, ifreq, nnt
complex w
real, allocatable, dimension(:) :: source, wk, nwk
complex, allocatable, dimension(:) :: csource, u, f, matl, matd, matr
...
allocate(source(nt), matl(nx), matd(nx), matr(nx))
allocate(csource(nt), u(nnt))
allocate(iwk(6*nt+150), wk(6*nt+150))
allocate(niwk(6*nnt+150), nwk(6*nnt+150))
allocate(green(nx,nf))
allocate(f(nx))


open(10,file='1DFDM_F.dat',access='direct',form='unformatted',recl=4*nnt)

call fdgaus(source,fmax,dt,nt)

do it=1,nt
   csource(it)=cmplx(source(it),0.)*exp(-alpha*it*dt)
end do
...
do ifreq=2,nf
   ...
   matd(1)=cmplx(0.,-1.)*w/c+1./dx
   matr(1)=-1./dx
   matl(nx)=-1./dx
   matd(nx)=cmplx(0.,-1.)*w/c+1./dx

   call CSY(1,nx,matl,matd,matr,f)
...
do ix=1,nx
    u(:)=0
    do ifreq=2,nf
        u(ifreq)=green(ix,ifreq)*csource(ifreq)
    enddo
...
! subroutines -  tridiagonal matrix solver, fdgaus, gauss, fftcc
!
!
      subroutine CSY(il,iu,bb,dd,aa,cc)
      complex aa(1),bb(1),cc(1),dd(1),r
! ... Tridiagonal equation solver following Thomas algorithm. - COMPLX
! version
! ... IL : Subscript of first equation
! ... IU : Subscript of last equation
! ... BB : coefficeint to the left of the main diagonal
! ... DD : coefficeint on the main diagonal
! ... AA : coefficeint to the right of the main diagonal
! ... CC : Element of the constant vector
!
